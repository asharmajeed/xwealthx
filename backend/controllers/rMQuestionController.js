import { RMQuestion } from "../models/questionModel.js";
import { v4 as uuidv4 } from "uuid"; // Import UUID generator
import { setQuizSession, getQuizSession } from "../utils/cacheHelpers.js";

export const addQuestion = async (req, res) => {
  try {
    const question = new RMQuestion(req.body);
    await question.save();
...
export const updateQuestion = async (req, res) => {
  const { id } = req.params;
  try {
    const question = await RMQuestion.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!question)
...
export const deleteQuestion = async (req, res) => {
  const { id } = req.params;
  try {
    const question = await RMQuestion.findByIdAndDelete(id);
    if (!question)
      return res.status(404).json({ message: "Question not found" });
...
export const getAllQuestions = async (req, res) => {
  try {
    const questions = await RMQuestion.find();
...
export const getFirst30Questions = async (req, res) => {
  try {
    const questions = await RMQuestion.find().limit(30);
    res.status(200).json(questions);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
export const getRandom85Questions = async (req, res) => {
  try {
    const questions = await RMQuestion.aggregate([{ $sample: { size: 85 } }]);
    res.status(200).json(questions);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
export const getAllQuestionsRandomOrder = async (req, res) => {
  try {
    ...
    const totalQuestions = await RMQuestion.countDocuments();
    const questions = await RMQuestion.aggregate([{ $sample: { size: totalQuestions } }]);
