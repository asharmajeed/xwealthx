...
export const getFirst30Questions = async (req, res) => {
  try {
    const questions = await IPgfQuestion.find().limit(30);
    res.status(200).json(questions);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
export const getAllQuestionsRandomOrder = async (req, res) => {
  try {
    ...
    // Fetch random questions using MongoDB aggregation
    const totalQuestions = await IPgfQuestion.countDocuments();
    const questions = await IPgfQuestion.aggregate([{ $sample: { size: totalQuestions } }]);
