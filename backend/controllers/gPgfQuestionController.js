...
export const addQuestion = async (req, res) => {
  try {
    const question = new GPgfQuestion(req.body);
    await question.save();
...
export const updateQuestion = async (req, res) => {
  const { id } = req.params;
  try {
    const question = await GPgfQuestion.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!question)
...
export const deleteQuestion = async (req, res) => {
  const { id } = req.params;
  try {
    const question = await GPgfQuestion.findByIdAndDelete(id);
    if (!question)
      return res.status(404).json({ message: "Question not found" });
...
export const getAllQuestions = async (req, res) => {
  try {
    const questions = await GPgfQuestion.find();
...
export const getAllQuestionsRandomOrder = async (req, res) => {
  try {
    ...
    const totalQuestions = await GPgfQuestion.countDocuments();
    const questions = await GPgfQuestion.aggregate([{ $sample: { size: totalQuestions } }]);
