import { TPgfQuestion } from "../models/questionModel.js";
import { v4 as uuidv4 } from "uuid"; // Import UUID generator
import { setQuizSession, getQuizSession, setExamSession, getExamSession } from "../utils/cacheHelpers.js";

export const addQuestion = async (req, res) => {
  try {
    const question = new TPgfQuestion(req.body);
    await question.save();
...
export const updateQuestion = async (req, res) => {
  const { id } = req.params;
  try {
    const question = await TPgfQuestion.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!question)
...
export const deleteQuestion = async (req, res) => {
  const { id } = req.params;
  try {
    const question = await TPgfQuestion.findByIdAndDelete(id);
    if (!question)
      return res.status(404).json({ message: "Question not found" });
...
export const getAllQuestions = async (req, res) => {
  try {
    const questions = await TPgfQuestion.find();
...
export const getFirst30Questions = async (req, res) => {
  try {
    const questions = await TPgfQuestion.find().limit(30);
    res.status(200).json(questions);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
export const getRandom85Questions = async (req, res) => {
  try {
    ...
    const questions = await TPgfQuestion.aggregate([{ $sample: { size: 85 } }]);
...
export const getAllQuestionsRandomOrder = async (req, res) => {
  try {
    ...
    // Fetch random questions using MongoDB aggregation
    const totalQuestions = await TPgfQuestion.countDocuments();
    const questions = await TPgfQuestion.aggregate([{ $sample: { size: totalQuestions } }]);
